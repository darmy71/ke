...
LOGS = logging.getLogger("اعداد كيبثون")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@kepthon"))
        await sbb_b(JoinChannelRequest("@kepthon_help"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                ...
                caption="**تم تشغيل سورس كيبثون بنجاح لعرض الاوامر ارسل `.الاوامر`**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/kepthon"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كروب بوت كيبثون", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
